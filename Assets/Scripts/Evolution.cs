...
{   
    ...
    void fitnessEvaluation() {
        ...
        if(distAux < GetComponent<CreateScene>().distJoints) {
            bestFitness = 1/Vector3.Distance(simulatedArm(robotState), goal.transform.position) - 1/Mathf.Pow(distAux, 2); //simplified the calculation and have unlimited both parts of the fitness calculation
        ...
        for (int i = 0; i < popSize; i++){
            ...
            if(considerObstacles){
                ...
                fitness = 1/Vector3.Distance(simulatedArm(popStates[i]), goal.transform.position) - 1/Mathf.Pow(distAux2, 2);
                print("b: " + 1/Mathf.Pow(distAux2, 2));
            }
            else
